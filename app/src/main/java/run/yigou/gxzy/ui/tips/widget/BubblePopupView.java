...
public void showPopupListWindow(View anchorView, int contextPosition, float rawX, float rawY,
    ...
    // 如果监听器存在且不允许显示弹出窗口，则直接返回
    if (mPopupListListener != null && !mPopupListListener.showPopupList(mContextView, mContextView, contextPosition)) {
        return;
    }
...
private void showPopupListWindow(float offsetX, float offsetY) {
    ...
    if (mIndicatorView != null) {
        adjustIndicatorPosition(location, offsetX);
    }
    ...
    // 防止弹出窗口超出屏幕边界
    x = Math.max(0, Math.min(x, mContext.getResources().getDisplayMetrics().widthPixels - mPopupWindowWidth));
    y = Math.max(0, Math.min(y, mContext.getResources().getDisplayMetrics().heightPixels - mPopupWindowHeight));
 ...
 * 填充弹出菜单列表
 */
private void populatePopupList(LinearLayout contentView) {
    ...
    popupListContainer.setOrientation(LinearLayout.VERTICAL);
    popupListContainer.setBackgroundDrawable(mCornerBackground);

    if (mIndicatorView != null) {
        manageIndicatorView(popupListContainer, contentView);
    }

    addItemsToPopupList(popupListContainer);
 ...
 * 管理指示器视图
 */
private void manageIndicatorView(LinearLayout popupListContainer, LinearLayout contentView) {
 ...
 * 添加项目到弹出列表
 */
private void addItemsToPopupList(LinearLayout popupListContainer) {
 ...
 * 设置项目背景
 */
private void setItemBackground(TextView textView, int index) {
 ...
 * 添加分割线
 */
private void addDivider(LinearLayout popupListContainer) {
 ...
 * 初始化弹出窗口
 */
private void initializePopupWindow(LinearLayout contentView) {
    ...
    if (mPopupWindowHeight == 0) {
        mPopupWindowHeight = getViewHeight(contentView) + mIndicatorHeight;
    }
 ...
 * 调整指示器的位置
 */
private void adjustIndicatorPosition(int[] location, float offsetX) {
