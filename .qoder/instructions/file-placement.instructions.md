---
description: "涉及新建、移动、重命名文件或目录时加载本文件。覆盖文件放置规则。"
name: "File Placement (Qoder)"
---
...
- 若新增的是模块级共享基础设施能力文件，应优先放入该模块的 `src/main/java` 对应包路径下，并保持模块归属清晰。
- 若现有目录已有明确归类和命名约定，必须优先复用，例如 `docs/`、`library/<module>/src/main/java`、模块目录 README 等。
...
- 对 README、模板资产、说明文档等约定文件，优先使用现有约定名称，不得自创平行命名。
